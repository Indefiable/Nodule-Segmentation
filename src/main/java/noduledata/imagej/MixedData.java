	 ...
	 */
	 public void findMixedNodules(ImagePlus im, ColorData red, ColorData green) {
	    	 ...
	    	 for(ShapeRoi roi : this.getRois()) {
    		 	...
    		 	//updateClumpedRois(index, red, green);
...
//   ==================================
	...
	private double calculateClosestDistance(Polygon polygon1, Polygon polygon2) {
	            ...
	            /*
	            // Get the boundary paths of the polygons
	            PathIterator pathIterator1 = polygon1.getPathIterator(null);
	            PathIterator pathIterator2 = polygon2.getPathIterator(null);

	            double[] coords1 = new double[6];
	            double[] coords2 = new double[6];
	            double[] prevCoord1 = new double[2];
	            double[] prevCoord2 = new double[2];

	            while (!pathIterator1.isDone()) {
	                int segmentType1 = pathIterator1.currentSegment(coords1);
	                int segmentType2 = pathIterator2.currentSegment(coords2);

	                if (segmentType1 == PathIterator.SEG_LINETO && segmentType2 == PathIterator.SEG_LINETO) {
	                    double dist = lineSegmentDistance(prevCoord1[0], prevCoord1[1], coords1[0], coords1[1], prevCoord2[0], prevCoord2[1], coords2[0], coords2[1]);
	                    minDistance = Math.min(minDistance, dist);
	                }

	                prevCoord1[0] = coords1[0];
	                prevCoord1[1] = coords1[1];

	                prevCoord2[0] = coords2[0];
	                prevCoord2[1] = coords2[1];
