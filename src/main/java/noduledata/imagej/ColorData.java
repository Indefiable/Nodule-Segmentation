 ...
 */
	...
	public void RemoveData(int[] indices) {
		...
		// remove in descending order to not fuck up indices of rois[].
		for (int ii = indices.length-1; ii >=0; ii--) {
...
/** Creates an annotated version of the map and displays it.*/
	...
	/** 
	 * measures the areas of the nodules.
	 */
	 ...
	 * @param clean: true to remove noisy data, false to keep it.
	 */
	public void getData(boolean mixed) {
		...
		/**
		 * next line returns an roi of all segmented pieces. Highlights all of the pieces accurately.
		 * This is where a core issue of the current pipeline is. This algorithm just separates by 
		 * a color threshold, it won't separate nodules that are together.
		 *  
		 * Proposed solution: devise some preprocessing algorithm that will create a line between nodules,
		 * separating them so this algo correctly identifies separate nodules. This can be done using 
		 * the shading in the image and the circularity of the nodules.
		 * 
		 * 
		...
		cleanData(); // indexOutOfBoundsError
...
// ====================================
/**
 * Finds noisy roi's and removes them from the map, resultsTable, roi array*/
	void cleanData() {
 ...
 */
	void fillNoduleHoles() {
		...
		while(ii++ < rois.length-1) {
			...
			//System.out.println(ii + ": " + percentArea);
 ...
 */
	...
	int stDev(double average) {
		...
		for ( int index : singleNodulesIndices) {
			...
			//System.out.println(Math.abs(average - area));
			stdev += ((average - area) * (average - area));
		}
		stdev = stdev / singleNodulesIndices.length;
		stdev =  Math.sqrt( (double) stdev);
		//System.out.println("avg: " + average + ", stdev: " + stdev);
		
		//this.HIGHAREATHRESHOLD = average + stdev;
		return (int) stdev;
	}
